...
const ListFilm = (props: NextPageWithLayout) => {
  const { data: films, error ,removeFilms } = useFilms();
      ...
      <List
        ...
        renderItem={(films: any, index) => (
          ...
          >
            ...
            <Link href={`/admin/films/${films.id}`}>Sửa</Link>
              </Button>
            <Button type="primary" danger style={{ marginLeft: 8 }} icon={<DeleteOutlined/>}  onClick= {()=> removeFilms(films.id)}>
              Xóa
            </Button>
          </List.Item>
        )}
